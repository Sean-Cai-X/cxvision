...
$PHASE_DIR = "$RUN_ROOT\analysis\$RUN_ID"
New-Item -ItemType Directory -Path $PHASE_DIR -Force | Out-Null

Write-Host "===== Parameter Analysis for Image Top Edge ====="
Write-Host "Run ID: $RUN_ID"
Write-Host "Optimal ROI: ($BASE_X0,$BASE_Y0)-($BASE_X1,$BASE_Y1)"
Write-Host "Image Feature: High contrast horizontal edge at top of black rectangle"
Write-Host "Goal: Reduce noise points while maintaining edge detection"
Write-Host ""
...
$results | Out-File "$PHASE_DIR\analysis_results.csv" -Encoding utf8

Write-Host ""
Write-Host "=== Top Candidates (points between 500-1500, avgdist < 25) ==="
$results | ConvertFrom-Csv | Where-Object { 
    $_.fit -eq "True" -and 
    [int]$_.points -ge 500 -and 
    [int]$_.points -le 1500 -and 
    [double]$_.avgdist -lt 25 
} | Sort-Object -Property avgdist, points | Select-Object -First 10

Write-Host ""
Write-Host "===== Analysis Complete ====="
Write-Host "Results written to: $PHASE_DIR\analysis_results.csv"