...
Write-Host "===== Findline Extension Test v2 ====="
...
$results = @()
$results += "name,y,threshold,wgap,hgap,linegap,method,filterprofile,points,fit,avgdist,failure_stage"
...
foreach ($test in $tests) {
    ...
    if (Test-Path $summaryPath) {
        try {
            ...
            if ($summary.failure_stage) { $FAILURE_STAGE = $summary.failure_stage }
        } catch {}
    }
    
    $fit_str = "false"
    if ($FIT -eq $true) { $fit_str = "true" }
    $results += $CASE_ID + "," + $test.Y + "," + $test.Threshold + "," + $test.Wgap + "," + $test.Hgap + "," + $test.Linegap + "," + $test.Method + "," + $test.Filterprofile + "," + $POINTS + "," + $fit_str + "," + $AVGDIST + "," + $FAILURE_STAGE
    
    Write-Host ("  -> points=" + $POINTS + ", fit=" + $fit_str + ", avgdist=" + $AVGDIST + ", failure=" + $FAILURE_STAGE)
    Write-Host ""
}
...
Write-Host "===== Results ====="
$results | ConvertFrom-Csv | Format-Table name, points, fit, avgdist, failure_stage
