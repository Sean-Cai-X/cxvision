$EXE = "D:\Codex-WorkDir\Sean_WorkDir\cxvisionai\build01\Release\cxvision_imgui_acceptance.exe"
$REPO = "D:\Codex-WorkDir\Sean_WorkDir\cxvisionai\cxvision_repo"
$IMAGE = "D:\Codex-WorkDir\Sean_WorkDir\cxvisionai\test_images\L1_high_contrast\line_high_contrast_001.jpg"
$SCRIPT = "$REPO\cxparser\cxscript\module\cximage\stage25\param_regression\param_self_tuning_findline_direct.cxsc"
$RUN_ROOT = "D:\Codex-WorkDir\Sean_WorkDir\cxvisionai\cxscript_runs\param_self_tuning"
$RUN_ID = "run_$(Get-Date -Format 'yyyyMMdd_HHmmss')"

$BASE_X0 = 82
$BASE_Y0 = 180
$BASE_X1 = 1210
$BASE_Y1 = 180

$BASE_METHOD = 2
$BASE_TOOL_HALF_WIDTH = 20

$PHASE_DIR = "$RUN_ROOT\comprehensive\$RUN_ID"
New-Item -ItemType Directory -Path $PHASE_DIR -Force | Out-Null

Write-Host "===== Comprehensive Parameter Test ====="
Write-Host "Run ID: $RUN_ID"
Write-Host "ROI: ($BASE_X0,$BASE_Y0)-($BASE_X1,$BASE_Y1)"
Write-Host "Image Feature: High contrast horizontal edge"
Write-Host "Testing: threshold, wgap, hgap, linegap, filterprofile, objfilter"
Write-Host ""

$results = @()
$results += "threshold,wgap,hgap,linegap,filterprofile,objfilter,points,fit,avgdist"

$best_points = 0
$best_avgdist = 999
$best_params = ""

$thresholds = @(20, 30, 40, 50)
$wgaps = @(5, 8, 11)
$hgaps = @(20, 32)
$linegaps = @(3, 6)
$filterprofiles = @(0, 1, 2)
$objfilters = @(0, 10, 20, 50)

$total = $thresholds.Count * $wgaps.Count * $hgaps.Count * $linegaps.Count * $filterprofiles.Count * $objfilters.Count
$count = 0

foreach ($threshold in $thresholds) {
    foreach ($wgap in $wgaps) {
        foreach ($hgap in $hgaps) {
            foreach ($linegap in $linegaps) {
                foreach ($filterprofile in $filterprofiles) {
                    foreach ($objfilter in $objfilters) {
                        $count++
                        $CASE_ID = "t${threshold}_w${wgap}_h${hgap}_l${linegap}_f${filterprofile}_o${objfilter}"
                        $OUT_DIR = "$PHASE_DIR\$CASE_ID"
                        New-Item -ItemType Directory -Path $OUT_DIR -Force | Out-Null
                        
                        Write-Host "Running [$count/$total]: t=$threshold, w=$wgap, h=$hgap, l=$linegap, f=$filterprofile, o=$objfilter"
                        
                        & $EXE `
                            --cxscript-headless `
                            --image $IMAGE `
                            --script $SCRIPT `
                            --out $OUT_DIR `
                            --case-name $CASE_ID `
                            --roi-x0 $BASE_X0 `
                            --roi-y0 $BASE_Y0 `
                            --roi-x1 $BASE_X1 `
                            --roi-y1 $BASE_Y1 `
                            --threshold $threshold `
                            --wgap $wgap `
                            --hgap $hgap `
                            --linegap $linegap `
                            --method $BASE_METHOD `
                            --filterprofile $filterprofile `
                            --tool-half-width $BASE_TOOL_HALF_WIDTH
                        
                        $POINTS = 0
                        $FIT = $false
                        $AVGDIST = 0.0
                        
                        $summaryPath = "$OUT_DIR\result_summary.json"
                        if (Test-Path $summaryPath) {
                            try {
                                $summary = Get-Content $summaryPath -Raw | ConvertFrom-Json
                                $POINTS = $summary.valid_points_count
                                $FIT = $summary.has_fit_line
                                $AVGDIST = $summary.avgdist
                            } catch {
                                Write-Host "Error parsing $summaryPath"
                            }
                        }
                        
                        $results += "$threshold,$wgap,$hgap,$linegap,$filterprofile,$objfilter,$POINTS,$FIT,$AVGDIST"
                        Write-Host "  -> points=$POINTS, fit=$FIT, avgdist=$AVGDIST"
                        
                        if ($FIT -eq $true -and $POINTS -ge 50 -and $POINTS -le 500 -and $AVGDIST -lt $best_avgdist) {
                            $best_avgdist = $AVGDIST
                            $best_points = $POINTS
                            $best_params = "threshold=$threshold, wgap=$wgap, hgap=$hgap, linegap=$linegap, filterprofile=$filterprofile, objfilter=$objfilter"
                        }
                    }
                }
            }
        }
    }
}

$results | Out-File "$PHASE_DIR\comprehensive_results.csv" -Encoding utf8

Write-Host ""
Write-Host "=== Best Parameters ==="
Write-Host "$best_params"
Write-Host "points=$best_points, avgdist=$best_avgdist"

Write-Host ""
Write-Host "===== Comprehensive Test Complete ====="
Write-Host "Results written to: $PHASE_DIR\comprehensive_results.csv"