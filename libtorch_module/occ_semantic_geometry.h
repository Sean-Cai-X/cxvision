...
public:
    ...
    virtual SemanticGeometryVector build(
        const gp_Path & path,
...
public:
    ...
    SemanticGeometryVector build(
        ...
        const float closed_flag = start_end_dist <= std::max(2.0f, path_len * 0.02f) ? 1.0f : 0.0f;
        ...
        const float aspect_ratio = occ_semantic_geometry::safe_div(box_w, std::max(box_h, 1e-6f));
        const float circularity = occ_semantic_geometry::safe_div(
            4.0f * 3.1415926f * box_area,
            std::max(path_len * path_len, 1e-6f));

        out.values[0] = occ_semantic_geometry::clamp01(static_cast<float>(count) / 512.0f);
        out.values[1] = occ_semantic_geometry::safe_div(path_len, std::max(img_w, img_h));
        ...
        out.values[6] = occ_semantic_geometry::safe_div(path_len, std::max(box_area, 1e-6f));
        ...
        out.values[11] = occ_semantic_geometry::safe_div(start_end_dist, std::max(img_w, img_h));
        out.values[12] = closed_flag;
        out.values[13] = occ_semantic_geometry::safe_div(mean_seg, std::max(img_w, img_h));
        out.values[14] = occ_semantic_geometry::safe_div(std_seg, std::max(img_w, img_h));
        out.values[15] = occ_semantic_geometry::safe_div(max_seg, std::max(img_w, img_h));
        out.values[16] = occ_semantic_geometry::safe_div(min_seg, std::max(img_w, img_h));
        out.values[17] = std::max(out.values[2], out.values[3]);
        out.values[18] = std::min(out.values[2], out.values[3]);
        out.values[19] = occ_semantic_geometry::safe_div(out.values[18], std::max(out.values[17], 1e-6f));

        const float orientation = std::atan2(
            static_cast<float>(last.Y() - first.Y()),
        ...
        for (int i = 1; i + 1 < static_cast<int>(sampled.size()); ++i) {
            ...
            const int turn_bin = std::min(7, static_cast<int>(angle / 3.1415926f * 8.0f));
            turn_hist[turn_bin] += 1.0f;

            const float curvature = angle / static_cast<float>(std::max((len_a + len_b) * 0.5, 1e-6));
            const int curv_bin = std::min(7, static_cast<int>(std::min(curvature, 1.0f) * 8.0f));
        ...
        float turn_sum = 0.0f;
        float curv_sum = 0.0f;
        for (float x : turn_hist) turn_sum += x;
        for (float x : curv_hist) curv_sum += x;
        ...
        out.values[40] = occ_semantic_geometry::clamp01(static_cast<float>(corner_count) / 16.0f);
        out.values[42] = (aspect_ratio > 5.0f || aspect_ratio < 0.2f) ? 1.0f : 0.0f;
        out.values[43] = closed_flag > 0.5f ? occ_semantic_geometry::clamp01(circularity) : 0.0f;
        out.values[44] = closed_flag > 0.5f
            ? occ_semantic_geometry::clamp01(1.0f - std::fabs(out.values[19] - 0.7f))
            : 0.0f;
        out.values[45] = closed_flag > 0.5f
            ? occ_semantic_geometry::clamp01(1.0f - std::fabs(aspect_ratio - 1.0f))
            : 0.0f;

        out.values[80] = occ_semantic_geometry::safe_div(static_cast<float>(ctx.roi_x), img_w);
        out.values[81] = occ_semantic_geometry::safe_div(static_cast<float>(ctx.roi_y), img_h);
        out.values[82] = occ_semantic_geometry::safe_div(static_cast<float>(ctx.roi_w), img_w);
        out.values[83] = occ_semantic_geometry::safe_div(static_cast<float>(ctx.roi_h), img_h);
        out.values[84] = occ_semantic_geometry::safe_div(center_x - img_w * 0.5f, img_w);
        out.values[85] = occ_semantic_geometry::safe_div(center_y - img_h * 0.5f, img_h);
        out.values[86] = occ_semantic_geometry::clamp01(static_cast<float>(ctx.state_id) / 32.0f);
        out.values[87] = occ_semantic_geometry::clamp01(static_cast<float>(ctx.camera_id) / 16.0f);
