...
$paramSets = @(
    @{ Name = "fmin1"; FilterMin = 1; Threshold = 20; Linegap = 6; Method = 1; FilterProfile = 0 },
    @{ Name = "fmin10"; FilterMin = 10; Threshold = 20; Linegap = 6; Method = 1; FilterProfile = 0 },
    @{ Name = "fmin20"; FilterMin = 20; Threshold = 20; Linegap = 6; Method = 1; FilterProfile = 1 },
    @{ Name = "thresh8"; FilterMin = 1; Threshold = 8; Linegap = 6; Method = 1; FilterProfile = 0 },
    ...
    @{ Name = "linegap10"; FilterMin = 1; Threshold = 20; Linegap = 10; Method = 1; FilterProfile = 0 },
    @{ Name = "thresh30"; FilterMin = 1; Threshold = 30; Linegap = 6; Method = 1; FilterProfile = 0 }
)
...
foreach ($testCase in $testCases) {
    ...
    foreach ($y in $yRange) {
        foreach ($paramSet in $paramSets) {
            $caseKey = "$caseName`_y$y`_$($paramSet.Name)"
            $caseDir = "$outputDir\$caseKey"
            
            try {
                New-Item -ItemType Directory -Path $caseDir -Force -ErrorAction Stop | Out-Null
            } catch {
                Write-Host "  SKIP: Cannot create dir for $caseKey"
                continue
            }
...
try {
    $results | Export-Csv -Path "$outputDir\self_tuning_results.csv" -NoTypeInformation -ErrorAction Stop
    Write-Host "`nResults saved to $outputDir\self_tuning_results.csv"
} catch {
    Write-Host "`nCannot save results: $_"
}
...
foreach ($case in $testCases) {
    ...
    if ($caseResults.Count -gt 0) {
        ...
        Write-Host "  Output Dir: $($best.OutputDir)"
    } else {
