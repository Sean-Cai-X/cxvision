...
## CxVision v2.5 框架完整分析与 v2.6 更新框架建议

（基于 `codex/cxcore-integration` 分支代码与文档）
...
当前版本处于**「框架成型、链路初通、核心能力待闭环」**的阶段，开发策略已从 v2.4 的 “搭框架” 正式切换为 “冻结框架、减少新增层、用真实 Case 疏通验证现有链条”。

整体沿用 8 个逻辑域的分层架构，以 CxScript 为统一逻辑层，向下调度传统视觉算子与 Torch 深度学习算子，向上输出统一结果、证据与计量语义，形成了 “传统视觉 + 深度学习” 双轨融合的基础骨架。核心特征包括：

- **统一执行入口**：Manual / Headless / Suite / Torch 共享同一套算法执行入口与结果格式
- **双算子体系**：传统 Find*/FastMatch 几何算子 + Torch 深度学习算子并行接入
- **全链路证据**：从 Runtime 结果到几何投影、计量语义、人工审核的完整证据链
- **资产化管理**：模型、脚本、数据集、结果全部按 Manifest / Contract 体系管理

#### 2. 核心模块成熟度矩阵

基于代码定义的状态标准，分六条标准执行链梳理：

表格

| 执行链路 | 成熟度等级 | 核心已实现能力 | 现存核心缺口 |
| --- | --- | --- | --- |
| 传统视觉链（Find* / FastMatch / Gauge） | 核心可用，局部待完善 | FindLine/FindCircle/FindEllipse 全功能实现；FastMatch 完整链路 + 双证据结构体；Gauge 几何关系与容差判定 | FastMatch 与 GridPattern 融合未完成；Formfit 精密贴合全场景验证不足 |
| Torch 推理链（分割 / 检测） | 链路打通，语义待补 | DeepLab / YOLOv8 / EdgeSAM 推理全通；Instance Segmentation 基础输出完成；Production Dispatcher 路由正常 | 实例语义被压扁（仅输出 bbox/confidence，缺完整实例字段）；YOLOv8 几何头未进入损失闭环；模型兼容性问题 |
| 增量学习链 | 框架具备，绑定待完成 | Prototype 增量生命周期实现；增量包校验框架完成；新增模型增量分支工作流 | EdgeSAM / YOLO 增量包真实权重未绑定；TorchTask 类型映射缺失增量 / 原型字段；GUI 开发父模型入口未开放 |
| 计量语义链 | 框架成型，核心待绑定 | 13 项 sidecar 语义证据输出；Surface Analytics / Boundary Trigger 接入 Find* 链路；CxCalibration 坐标变换框架 | Calibration 未绑定真实标定数据；Pattern Model 未绑定；真值精度缺失；不确定性预算未闭环 |
| 证据审核链 | 结构完整，验收待推进 | 快照 / 摘要 / 追踪 / 叠加 / 回放 / 合同 / 审核门全实现；Torch 证据链分级（T3-T7）；F9 工作流投影 | 多数模块处于 Verification Pending；人工审核全链路未走通；Promotion 功能禁用 |
| 构建运行链 | 基础稳定，加速待扩展 | CMake 完整构建；Windows MSVC 编译通过；Torch Runtime DLL 集成；统一日志 / 崩溃日志 | geometry_accel 硬件加速模块未落地；CUDA 设备合同 Partial；Linux 编译验证不足 |

#### 3. 当前核心断点与瓶颈

1. **几何能力断点**
YOLOv8 的 Geometry Head 仅完成张量结构，未进入损失函数训练闭环（状态为 `prepared_not_loss_consumed`），原生 OBB 本质是统计拟合，旋转鲁棒性差、精度漂移，无法作为生产级几何输出源。
2. **双轨通道断点**
开发验证通道的语义定义已明确，但 GUI 训练入口仅支持正式 release 父模型，`DEVELOPMENT_ONLY` 状态的开发父模型无独立入口，导致业务小样本验证无法从界面直接启动。
3. **结果语义瓶颈**
Canonical Result 契约跟不上模型能力，Instance Segmentation 的完整实例语义被压扁为普通检测结构，模型输出的轮廓、角度、不确定性等字段无法透传到下游几何与计量层。
4. **计量闭环缺口**
Calibration、Pattern Model、Ground Truth 全部处于未绑定状态，计量语义只有框架，没有真实物理量标定，无法输出可溯源的测量结果。
5. **工具链错位**
FastMatch 作为几何精度兜底核心，当前仅作为传统视觉工具存在，未与 Torch 分割链路深度整合，也未形成可管理的模型库体系。
...
**定位**：双轨通道落地版 + FastMatch 几何化升级
**核心目标**：

- 落地开发 / 生产双隔离通道，解决业务快速验证与标准生产管控的矛盾
- 完成 FastMatch 模型化升级，构建「分割 Backbone + FM 几何头」新链路，替代 YOLOv8 OBB 统计拟合方案
- 补全几何计量闭环的核心节点，推动核心链路从 “可用” 走向 “可验收”

#### 2. 双轨通道落地框架

严格对齐开发隔离、生产严控的边界，形成体系化的通道设计：

#### （1）开发验证通道（DEVELOPMENT_ONLY）

- **准入资产**：标记为 `DEVELOPMENT_ONLY` 的七类候选父模型、业务小样本标注集、开发版 FastMatch 模型
- **核心能力**：小样本增量训练、快速推理验证、几何精度兜底、失败一键回滚
- **隔离规则**：所有子模型存入独立开发目录；不写入正式模型谱系；不进入生产推理接口；失败直接回滚不污染主库
- **入口配套**：GUI 新增独立的「开发验证入口」，支持选择开发父模型、绑定案例专属数据集、启动增量训练与验证
- **输出产物**：开发态子模型、验证报告、失败样本库、效果对标数据

#### （2）生产准入通道

- **准入标准**：完整 Geometry Head 训练闭环 + 正式谱系登记 + 迁移回执 + 固定验证 /holdout 集 + 人工审核
- **状态流转**：EVALUATED → APPROVED → ACTIVE
- **核心能力**：正式模型推理、全链路证据输出、计量溯源、版本可回滚
- **兜底机制**：FastMatch 作为生产级几何校准与异常降级模块，与模型几何头形成双保险

#### 3. FastMatch 模型化链路升级（核心替代方案）

将 FastMatch 从 “后处理工具” 升级为 “可管理的几何模型库”，构建替代 YOLOv8 OBB 的原生几何链路，分三层落地：

#### （1）算子层：GPU 加速化

- 核心工作：将 FastMatch 的轮廓提取、亚像素采样、椭圆 / 圆 / 直线最小二乘拟合、正交约束、残差计算等核心算子移植为 CUDA 实现，兼容 SYCL
- 接入位置：串联在 Torch 分割 Mask 输出之后，替代原 CPU 侧后处理，实现 GPU 端到端计算
- 核心收益：消除 CPU/GPU 数据拷贝开销，推理速度量级提升，精度与 CPU 版完全一致

#### （2）模型层：专属模型库体系

- **模型定义**：每个业务场景 / 目标类别对应一个 FastMatch 专属几何模型，包含「目标轮廓特征 + 几何约束 + 场景适配参数」
- **管理体系**：和神经网络权重统一纳入模型资产管理，支持版本、哈希、回滚、状态标记（开发 / 正式）
- **训练生成**：基于标注样本自动完成特征标定、约束拟合、参数寻优，分钟级生成模型，无需梯度训练
- **两级库结构**：
  - 基准模型库：七类几何各一个通用基准模型，用于未知场景粗匹配
  - 业务专属模型库：每个具体业务场景一个优化模型，用于高精度生产

#### （3）链路层：替代 YOLOv8 OBB 链路

- 新链路结构：**YOLOv8 分割 Backbone → Mask 输出 → FastMatch 几何头 → 标准几何参数输出**
- 替代逻辑：保留 YOLOv8 的特征提取与轮廓分割能力，废掉其 OBB 回归头，几何计算全部交给 FastMatch 闭式求解
- 核心优势：从根上解决统计拟合的旋转漂移问题，几何精度由数学闭式解保证，训练效率、推理速度、可解释性全面优于原生 OBB

#### 4. 几何与计量链路闭环

1. **几何投影层补全**：补全 Instance Segmentation 到几何形状的完整投影，支持轮廓、中心、角度、长短轴等全字段透传，解决结果语义压扁问题
2. **标定绑定**：支持 CxCalibration 绑定真实标定板数据，输出像素→物理坐标的真实转换参数，完成计量溯源基础
3. **基准对接**：FastMatch 几何输出直接对接 Measurement Semantics 层，生成符合规范的计量证据与不确定性评估
4. **异常降级机制**：生产链路中，当模型几何头置信度低于阈值时，自动降级为 FastMatch 纯轮廓拟合模式，保证测量不中断

#### 5. 证据链与版本治理补全

1. **开发通道证据链**：补全开发模型的训练日志、验证报告、失败样本、回滚记录，形成独立的开发证据链，不与生产证据混淆
2. **模型版本治理**：明确父子模型的版本映射、哈希校验、状态流转规则，增量训练必须从上一正式版本迭代，禁止覆盖
3. **人工审核节点**：开发验证转生产必须经过人工审核节点，核对精度、鲁棒性、证据完整性后才能晋升
4. **可追溯性**：所有几何结果都带来源标记（模型几何头 / FastMatch 校正），支持全链路溯源

---

### 三、文档说明修正要点（供 Trae 执行）

#### 1. 状态与定义修正

- 新增 `DEVELOPMENT_ONLY` 父模型状态定义，明确其权限、边界、适用场景与隔离规则
- 新增 FastMatch 几何模型的状态定义与资产管理规范
- 修正 Geometry Head 的状态描述，明确 `prepared_not_loss_consumed` 的含义、能力边界与适用阶段

#### 2. 架构章节补充

- 在六条标准执行链中，新增「FastMatch 几何模型链」的完整链路说明
- 补充双轨通道（开发验证 / 生产准入）的架构设计、状态流转与边界规则
- 更新几何链路的定位说明，明确「分割 Backbone + FM 几何头」的新范式，以及对原生 OBB 方案的替代关系

#### 3. 状态矩阵更新

- 更新 FastMatch 模块状态为「Implemented + 模型化扩展中」
- 新增开发通道相关模块的状态项与进度说明
- 修正几何头、计量绑定相关模块的状态描述，匹配当前真实进度

#### 4. 核心原则补充

- 新增**几何优先原则**：工业计量场景下，几何闭式求解优先于统计回归；模型负责特征提取，几何负责精度兜底
- 新增**双轨隔离原则**：开发与生产通道严格隔离，开发资产不污染生产体系，生产标准不阻塞开发迭代
- 新增**模型资产统一原则**：神经网络权重与 FastMatch 几何模型统一纳入资产管理体系，共享版本、哈希、审核机制

#### 5. 推进路线更新

- 把 FastMatch 模型化、双轨通道落地列为下一阶段核心优先级
- 调整几何能力推进路径：优先落地 FastMatch 几何链路兜底，再推进模型原生 Geometry Head 训练闭环
- 补充增量训练与在线测量链隔离的具体执行规则
