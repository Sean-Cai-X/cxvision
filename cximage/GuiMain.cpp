...
bool PipelineFastMatchSearchContainsLearn(
    ...
    std::string& reason)
{
    ...
    const bool ok =
        PipelineParamInt(parameterSummary, "learn_roi_x", learnX) &&
    ...
    if (searchW <= learnW || searchH <= learnH)
    {
        reason =
            ...
            ", search=" +
            std::to_string(searchW) + "x" + std::to_string(searchH);
        return false;
    }
...
int RunEvidenceLockPipelineCli(const EvidenceChainSelfTestCliOptions& options)
{
    ...
    planned.push_back({makeReq("T_findellipse_locked", ellipseScript, "baseline_01", "ellipse_main", "FindEllipse", "method=1 threshold=8 gap=5 linegap=3 ellipse_x0=600 ellipse_y0=360 ellipse_x1=930 ellipse_y1=580 ellipse_inner_scale_percent=0 findellipse_findsetting=1"), "PARAM_BINDING_PASS", false, true});
    ...
    for (const PlannedCase& pc : planned)
    {
        ...
        const bool fastmatchParamOk =
            ...
            PipelineFastMatchSearchContainsLearn(
                pc.request.parameter_summary,
...
bool WriteStandardChainGateReports(
    const std::string& outDir,
    const std::string& runId,
    const std::vector<StandardChainGateRow>& rows,
    std::string& reason)
{
...

    {
        ...
        file << "- fail_count: " << fail << "\n\n";
        file << "| Priority | Title | Code | Status | Evidence | Remaining | Next Step |\n";
        file << "|---|---|---|---|---|---|---|\n";
        for (const auto& row : rows)
        {
            file << "| " << row.priority
        ...
        file << "\n";
        file << "Promotion is blocked unless P0-P4 are PASS and human review assets are accepted.\n";
    }
...
int RunStandardChainGateCli(const EvidenceChainSelfTestCliOptions& options)
{
    ...
    const std::string run_id = CxUnifiedLog::Instance().GenerateRunId();
    const std::string out_dir = options.out_dir.empty()
        ? "D:/Codex-WorkDir/Sean_WorkDir/cxvisionai/cxscript_runs/standard_chain_gate/run_" + run_id
        : options.out_dir;
    ...
    const bool evidencePass = evidenceExit == 0 &&
        std::filesystem::exists(
            std::filesystem::path(evidenceOptions.out_dir) /
    ...
    baselineOptions.suite_path = baselineOptions.suite_path.empty()
        ? "cxparser/cxscript/module/cximage/stage25/suites/stage25_l1_l3_parameter_consistency.cxsc"
        : baselineOptions.suite_path;
    baselineOptions.image_manifest_path = baselineOptions.image_manifest_path.empty()
    ...
    const bool baselinePass = baselineExit == 0 &&
        std::filesystem::exists(
            std::filesystem::path(baselineOptions.out_dir) /
    ...
    paramOptions.suite_path = paramOptions.suite_path.empty()
        ? "cxparser/cxscript/module/cximage/stage25/suites/stage25_l1_l3_parameter_consistency.cxsc"
        : paramOptions.suite_path;
    paramOptions.image_manifest_path = paramOptions.image_manifest_path.empty()
        ? "cxparser/cxscript/module/cximage/stage25/manifests/stage25_l1_l3_manifest.json"
        : paramOptions.image_manifest_path;
    paramOptions.param_regression_tool = paramOptions.param_regression_tool.empty()
        ? "FindLine"
    ...
    const bool paramRegressionLoopPass = paramExit == 0 &&
        std::filesystem::exists(
    ...
    const std::filesystem::path hitReportPath =
        std::filesystem::path(paramOptions.out_dir) / "param_hit_distribution_report.md";
    const bool realHitDistribution =
        paramRegressionLoopPass &&
        std::filesystem::exists(hitJsonPath) &&
        std::filesystem::exists(hitReportPath) &&
        PipelineFileContainsText(hitJsonPath, "param_hit_distribution_runtime_v2") &&
        !PipelineFileContainsText(hitReportPath, "placeholder");
    const std::filesystem::path caseMatrixPath =
        std::filesystem::path(paramOptions.out_dir) / "candidate_case_matrix.json";
    ...
    const bool crossCaseMatrixReady =
        paramRegressionLoopPass &&
        std::filesystem::exists(caseMatrixPath) &&
        std::filesystem::exists(stabilityMatrixPath) &&
        PipelineFileContainsText(caseMatrixPath, "candidate_case_matrix_runtime_v1") &&
        PipelineFileContainsText(stabilityMatrixPath, "param_stability_runtime_v1") &&
        PipelineFileContainsText(stabilityMatrixPath, "\"coverage_complete\": true");
    ...
    rows.push_back({
        ...
        evidencePass
            ? "Evidence Lock Pipeline 18-case completed through Manual/Runtime/Evidence selftest path."
            : "Evidence Lock Pipeline failed or summary missing.",
        evidencePass ? "Manual GUI human review is still separate." : "Fix pipeline before expanding tools.",
        evidencePass ? "Use unified request/result gate for every new tool." : "Run --evidence-lock-pipeline and inspect failures."
    });
    rows.push_back({
        "P1",
        "固定 L1/L2/L3 基线",
        ...
        baselinePass ? "baseline_lock_summary.json confirms all L1/L2/L3 cases are locked." : "baseline_lock_summary.json reports missing or inconsistent L1/L2/L3 assets.",
        baselinePass ? "No remaining baseline lock blocker." : "Some suite image_id/target_id/script/expected assets are not locked.",
        baselinePass ? "Proceed to Candidate -> Probe -> EvalRecord -> Review." : "Fix manifest target ROI entries or suite bindings, then rerun --baseline-lock."
    });
    rows.push_back({
        "P2",
        "完成参数候选循环",
        paramRegressionLoopPass ? "STANDARD_CHAIN_P2_PASS" : "STANDARD_CHAIN_P2_FAIL",
        paramRegressionLoopPass ? "PASS" : "FAIL",
        paramRegressionLoopPass ? "param_regression_loop_summary.json confirms Candidate -> Probe -> EvalRecord -> Review-pending assets." : "Param regression loop failed or summary missing.",
        paramRegressionLoopPass ? "Mini-regression and human review are still separate." : "Fix --param-regression-loop before expanding candidate search.",
        paramRegressionLoopPass ? "Proceed to P3 real hit bins from runtime points." : "Run --param-regression-loop and inspect candidate case failures."
    });
    rows.push_back({
        "P3",
        "实现真实命中分布",
        realHitDistribution ? "STANDARD_CHAIN_P3_PASS" :
            (realCapture ? "STANDARD_CHAIN_P3_PARTIAL" : "STANDARD_CHAIN_P3_FAIL"),
        realHitDistribution ? "PASS" : (realCapture ? "PARTIAL" : "FAIL"),
        realHitDistribution ? "hit_distribution.json is generated from runtime measure_points shape snapshots." :
            (realCapture ? "Runtime result capture exists but hit_distribution is not generated from current runtime points." : "Runtime capture missing."),
        realHitDistribution ? "No P3 exporter blocker; zero points remain an algorithm/probe diagnostic if recorded." :
            "Need runtime hit bins from result_summary measure_points snapshots.",
        realHitDistribution ? "Proceed to P4 L2/L3 cross-case matrix." :
            "Populate hit bins from measure_points/valid_points/rejected_points snapshots."
    });
    rows.push_back({
        "P4",
        "完成跨案例准确性与稳定性",
        crossCaseMatrixReady ? "STANDARD_CHAIN_P4_PASS" : "STANDARD_CHAIN_P4_PENDING",
        crossCaseMatrixReady ? "PASS" : "PENDING",
        crossCaseMatrixReady
            ? "candidate_case_matrix.json and stability_matrix.json cover locked L1/L2/L3 cases with real EvalRecord rows."
            : "No complete locked L1/L2/L3 candidate-case stability matrix from current run.",
        crossCaseMatrixReady
    ...
    rows.push_back({
        "P5",
        "接入真实模型排序",
        "STANDARD_CHAIN_P5_PENDING",
        "PENDING",
        "Current ranking is rule/placeholder style; no verified historical model scorer.",
    ...
    rows.push_back({
        "P6",
        "接入真实优化目标",
        "STANDARD_CHAIN_P6_PENDING",
        "PENDING",
        "No verified ensmallen objective producing bounded candidates in this run.",
        "Need objective from real EvalRecord metrics.",
        "Optimizer may output candidates only; formal params remain locked until review."
    });
    rows.push_back({
        "P7",
        "统一模型结果和证据链",
        "STANDARD_CHAIN_P7_PARTIAL",
        "PARTIAL",
        "Torch/FastMatch result fields are entering Runtime/Evidence structures.",
        "Need every model result to publish Shape/Overlay/Evidence references through one projector.",
        "Add result_projector contract per model tool."
    });
    rows.push_back({
        "P8",
        "打通 Mini Regression 和 Promotion Gate",
        promotionGate ? "STANDARD_CHAIN_P8_PARTIAL" : "STANDARD_CHAIN_P8_FAIL",
        promotionGate ? "PARTIAL" : "FAIL",
        promotionGate ? "Promotion gate report exists in param regression exporter as blocked-by-default." : "Promotion gate output missing.",
        "Need real L2/L3 pass matrix plus accepted human review before promotion_allowed=true.",
    ...
    std::cout << "summary=" << (std::filesystem::path(out_dir) / "standard_chain_gate_summary.json").string() << "\n";
    std::cout << "report=" << (std::filesystem::path(out_dir) / "standard_chain_gate_report.md").string() << "\n";
...
const CxScriptCatalogEntry* PipelineFindCatalogEntry(
...
std::string PipelineTargetGeometryJson(const CxScriptImageTargetRoi& target)
{
    ...
    oss << ",\"wgap\":" << target.wgap
        << ",\"hgap\":" << target.hgap
        << ",\"gap\":" << target.gap
        << ",\"linegap\":" << target.linegap
...
bool PipelineWriteBaselineLockReports(
    ...
    const CxScriptSuiteRuntime& suite,
    const CxScriptCatalogRuntime& catalog,
    ...
    std::string& reason)
{
    ...
    for (const auto& c : suite.cases)
    {
        ...
        const CxScriptCatalogEntry* entry = PipelineFindCatalogEntry(catalog, c.script_id);
        if (!entry)
        {
            row.status = "FAIL";
        ...
        const CxScriptImageManifestEntry* image = FindImageById(manifest, c.image_id);
        if (!image)
        {
            row.status = "FAIL";
            if (!row.reason.empty())
        ...
        else
        {
            ...
            row.image_hash = !image->sha256.empty() ? ("manifest_sha256:" + image->sha256)
        ...
        else
        {
            ...
            if (!row.tool.empty() && !target->tool.empty() && row.tool != target->tool)
            {
                row.status = "FAIL";
                if (!row.reason.empty())
                    row.reason += "; ";
...

    {
        ...
        for (std::size_t i = 0; i < rows.size(); ++i)
        {
            ...
            file << "      \"target_geometry\": " << (r.target_json.empty() ? "{}" : r.target_json) << ",\n";
            file << "      \"expected_result\": \"" << PipelineJsonEscape(r.expected_result) << "\",\n";
            file << "      \"expected_policy_guard\": \"" << PipelineJsonEscape(r.expected_policy_guard) << "\",\n";
            file << "      \"contract_path\": \"" << PipelineJsonEscape(r.contract_path) << "\",\n";
...

    {
        ...
        file << "- pass_count: " << pass << "\n";
        file << "- fail_count: " << fail << "\n\n";
        file << "| Case | Level | Tool | Image | Target | Script | Expected | Status | Reason |\n";
        file << "|---|---|---|---|---|---|---|---|---|\n";
...
int RunBaselineLockCli(const EvidenceChainSelfTestCliOptions& options)
{
    const std::string suitePath = options.suite_path.empty()
        ? "cxparser/cxscript/module/cximage/stage25/suites/stage25_l1_l3_parameter_consistency.cxsc"
        : options.suite_path;
    const std::string manifestPath = options.image_manifest_path.empty()
        ? "cxparser/cxscript/module/cximage/stage25/manifests/stage25_l1_l3_manifest.json"
        : options.image_manifest_path;
    const std::string runId = CxUnifiedLog::Instance().GenerateRunId();
    const std::string outDir = options.out_dir.empty()
        ? "D:/Codex-WorkDir/Sean_WorkDir/cxvisionai/cxscript_runs/baseline_lock/run_" + runId
        : options.out_dir;
    ...
    const std::string catalogPath = suite.catalog_path.empty()
    ...
    bool lockPass = false;
    if (!PipelineWriteBaselineLockReports(
            outDir,
            runId,
            suitePath,
            manifestPath,
...
void PipelineApplyTargetToCandidate(
    const CxScriptImageTargetRoi& target,
...
void PipelineFillProbeGeometry(
    const CxScriptImageTargetRoi& target,
...
bool PipelineWriteParamRegressionLoopReports(
    ...
    const std::vector<ParamRegressionLoopRow>& rows,
    bool pass,
    std::string& reason)
{
    namespace fs = std::filesystem;
    ...
    {
        std::ofstream file(out / "param_regression_loop_summary.json", std::ios::binary);
        if (!file.is_open())
        {
        ...
        file << "{\n";
        file << "  \"run_id\": \"" << PipelineJsonEscape(runId) << "\",\n";
        file << "  \"final_code\": \"" << (pass ? "PARAM_REGRESSION_LOOP_PASS" : "PARAM_REGRESSION_LOOP_FAIL") << "\",\n";
        file << "  \"suite_path\": \"" << PipelineJsonEscape(suitePath) << "\",\n";
        file << "  \"manifest_path\": \"" << PipelineJsonEscape(manifestPath) << "\",\n";
        file << "  \"param_regression_script\": \"" << PipelineJsonEscape(paramScript) << "\",\n";
        ...
        for (std::size_t i = 0; i < rows.size(); ++i)
        {
            const auto& row = rows[i];
            file << "    {\n";
            file << "      \"candidate_id\": \"" << PipelineJsonEscape(row.candidate_id) << "\",\n";
            ...
            file << "      \"assets_complete\": " << (row.assets_complete ? "true" : "false") << ",\n";
            file << "      \"timeout\": " << (row.timeout ? "true" : "false") << ",\n";
            file << "      \"points\": " << row.points << ",\n";
            file << "      \"fit_available\": " << (row.fit_available ? "true" : "false") << ",\n";
            file << "      \"mean_distance\": " << row.mean_distance << ",\n";
            file << "      \"failure_stage\": \"" << PipelineJsonEscape(row.failure_stage) << "\",\n";
    ...
    {
        std::ofstream file(out / "param_regression_loop_report.md", std::ios::binary);
        if (!file.is_open())
        {
        ...
        file << "- review_status: PENDING_HUMAN_REVIEW\n";
        file << "- promotion_allowed: false\n\n";
        file << "| Candidate | Case | Launched | Executed | Assets | Timeout | Points | Fit | MeanDist | Status | Reason |\n";
        file << "|---|---|---|---|---|---|---:|---|---:|---|---|\n";
        for (const auto& row : rows)
        {
            file << "| " << row.candidate_id
                 << " | " << row.case_id
                 << " | " << (row.launched ? "yes" : "no")
                 << " | " << (row.executed ? "yes" : "no")
        ...
        file << "\n";
        file << "This report verifies the P2 chain only: Candidate -> Probe -> EvalRecord -> Review-pending.\n";
        file << "It does not promote parameters and does not replace L2/L3 stability or human review.\n";
    }
...
int RunParamRegressionLoopCli(const EvidenceChainSelfTestCliOptions& options)
{
    ...
    const std::string manifestPath = options.image_manifest_path.empty()
        ? "cxparser/cxscript/module/cximage/stage25/manifests/stage25_l1_l3_manifest.json"
        : options.image_manifest_path;
    const std::string paramScript = options.param_regression_script.empty()
        ? (tool == "FindCircle"
            ? "cxparser/cxscript/module/cximage/stage25/param_regression/ranges/findcircle_range_conservative.cxsc"
            : "cxparser/cxscript/module/cximage/stage25/param_regression/ranges/findline_range_conservative.cxsc")
        : options.param_regression_script;
    const std::string runId = CxUnifiedLog::Instance().GenerateRunId();
    const std::string outDir = options.out_dir.empty()
        ? ("D:/Codex-WorkDir/Sean_WorkDir/cxvisionai/cxscript_runs/param_regression_loop/run_" + runId)
        : options.out_dir;
    ...
    {
        std::cout << "param_regression_loop_ok=false\nreason=no locked baseline case found for tool=" << tool << "\n";
        return 2;
    }
    ...
    auto addFirstLevel = [&](const std::string& level)
    {
        ...
        for (const auto& locked : matchingCases)
        {
            ...
            {
                if (current.suite_case && current.suite_case->case_id == locked.suite_case->case_id)
                {
                    exists = true;
                    break;
                }
    ...
    runtime.task.target_id = anchor.suite_case ? anchor.suite_case->target_id : "";
    runtime.task.tool = tool;
    runtime.task.base_script_id = anchor.suite_case ? anchor.suite_case->script_id : "";
    runtime.task.base_parameter_profile_id = anchor.suite_case ? anchor.suite_case->expected_policy_guard : "";
    ...
    if (runtime.candidates.empty())
    {
        runtime.candidates = GenerateBasicParamCandidates(
            runtime.range_set,
            runtime.range_set.max_candidates);
    }
    ...
    for (const auto& candidate : runtime.candidates)
    {
        ...
        {
            if (!locked.suite_case || !locked.catalog_entry || !locked.image || !locked.target)
                continue;
            ...
            caseTask.base_parameter_profile_id = locked.suite_case->expected_policy_guard;
            ...
            probe.out_dir = (fs::path(outDir) / "cases" /
                caseCandidate.candidate_id / locked.suite_case->case_id).string();
            ...
            record.classification = probeResult.fit_available ? "geometry_available" : "geometry_unavailable";
    ...
    for (auto& s : stats)
    {
        const double total = s.total_cases > 0 ? static_cast<double>(s.total_cases) : 1.0;
        ...
        s.human_accept_rate = s.total_cases > 0
            ? static_cast<double>(s.human_accept) / total
            : 0.0;
    ...
    std::cout << "summary=" << (fs::path(outDir) / "param_regression_loop_summary.json").string() << "\n";
    std::cout << "report=" << (fs::path(outDir) / "param_regression_loop_report.md").string() << "\n";
    return loopPass ? 0 : 1;
}
